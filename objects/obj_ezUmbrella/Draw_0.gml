...
if (reserved){
	if (ownerType == "Friends"){
		...
		draw_sprite(spr_visitor_settled,8,x,y);
	} else if (ownerType == "Barbecuer"){
		draw_sprite(spr_visitor_settled,6,x,y);
	} else if (ownerType == "conservative"){
		draw_sprite(spr_visitor_settled,1,x,y);
	} else if (ownerType == "Poor"){
		draw_sprite(spr_visitor_settled,0,x,y);
	} else if (ownerType == "Creep"){
		draw_sprite(spr_visitor_settled,7,x,y);
	}
	...
	draw_text_transformed(x-sprite_xoffset+2,y-sprite_yoffset+1+topPlaceOffset,ownerType/*+" "+string(satisfactionScore)+" "+string(timeLeft)*/,0.75,0.75,0);
} else {
	if (obj_ezGameControl.visitorChoosen){
		if (mouseOver){
